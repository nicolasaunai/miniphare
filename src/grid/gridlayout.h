...
public:
    ...
    GridLayout(std::array<double, 3> dxdydz, std::array<uint32, 3> nbrCells, uint32 nbDims,
    ...
    double odx() const
    {
        return dx_ == 0. ? throw error(errorInverseMesh + " dx() (dx==0)") : odx_;
    }
    double ody() const
    {
        return dy_ == 0. ? throw error(errorInverseMesh + " dy() (dy==0)") : ody_;
    }
    double odz() const
    {
        return dz_ == 0. ? throw error(errorInverseMesh + " dz() (dz==0)") : odz_;
    }
    ...
    std::array<uint32, 3> nbrCellxyz() const { return {{nbrCellx_, nbrCelly_, nbrCellz_}}; }
    ...
    Point fieldNodeCoordinates(const Field& field, const Point& origin, uint32 ix, uint32 iy,
                               uint32 iz) const;
