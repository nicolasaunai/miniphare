...
::testing::AssertionResult AreVectorOfBfieldsEqual(const std::vector<VecField>& expected_vector,
            ...
            for (uint32 ix = 0; ix < actual_field.shape()[0]; ++ix)
            {
                if (expected_field.shape()[0] != actual_field.shape()[0])
                {
                    ...
                    sizeFailure << "Size of actual_field \n"
                                << "nx, ny, nz :" << actual_field.shape()[0] << ", "
                                << actual_field.shape()[1] << ", " << actual_field.shape()[2]
                                << "\n";

                    sizeFailure << "Size of expected_field \n"
                                << "nx, ny, nz :" << expected_field.shape()[0] << ", "
                                << expected_field.shape()[1] << ", " << expected_field.shape()[2]
                                << "\n";
...
public:
    ...
    void SetUp()
    {
        ...
        VecField Bold(allocBx, allocBy, allocBz,
                      {{HybridQuantity::Bx, HybridQuantity::By, HybridQuantity::Bz}}, "Bold");

        // B at time n+1/2
        VecField Bsol(allocBx, allocBy, allocBz,
                      {{HybridQuantity::Bx, HybridQuantity::By, HybridQuantity::Bz}}, "Bsol");

        // E at time n
        VecField Epred(allocEx, allocEy, allocEz,
                       {{HybridQuantity::Ex, HybridQuantity::Ey, HybridQuantity::Ez}}, "Epred");
        ...
        for (uint32 itime = 0; itime < inputs.nbrTimeSteps; ++itime)
        {
            ...
            for (uint32 ifield = 0; ifield < inputs.nbrOfFields; ++ifield)
            {
                ...
                std::string filename{"../Faraday/faraday1D_" + testName + "_" + fieldName + "_t"
                                     + std::to_string(itime) + ".txt"};
    ...
    void print(FaradayParams const& inputs)
    {
        std::cout << "interpOrder : " << inputs.interpOrder << " nbDims   : " << inputs.nbDim
                  << " nbr cell x = " << inputs.nbrCells[0]
                  << ", nbr cell y = " << inputs.nbrCells[1]
                  << ", nbr cell z = " << inputs.nbrCells[2] << " dx = " << inputs.dxdydz[0]
                  << ", dy = " << inputs.dxdydz[1] << ", dz = " << inputs.dxdydz[2]
                  << " dt = " << inputs.dt << " tStart = " << inputs.tStart
                  << " tEnd = " << inputs.tEnd << " testName = " << inputs.testName << std::endl;
    }
};
...
{
    EXPECT_TRUE(AreVectorOfBfieldsEqual(expected_Bfield, actual_Bfield, precision));
}
