/*
 ...
 10 - deriv()
    ...
    10.2 - calculate the derivative of the const Field along the direction and stores it in the dest
 field.
...
struct GridLayoutImplFactoryParams
{
    ...
    GridLayoutImplFactoryParams(uint32 nbDims, uint32 interpOrder, std::string const& layoutName,
                                std::array<uint32, 3> nbrCellsXYZ, std::array<double, 3> dxdydz)
        : nbDims_{nbDims}
        , interpOrder_{interpOrder}
        , layoutName_{layoutName}
        , nbrCellsXYZ_{nbrCellsXYZ}
        , dxdydz_{dxdydz}
    {
    }
...
TEST_P(GridLayoutImplFactoryTest, factoryParamTests)
{
    ...
    ASSERT_ANY_THROW(GridLayoutImplFactory::createGridLayoutImpl(
        inputs.nbDims_, Point{0., 0., 0.}, inputs.interpOrder_, inputs.layoutName_,
        inputs.nbrCellsXYZ_, inputs.dxdydz_));
}

INSTANTIATE_TEST_CASE_P(GridLayoutTest, GridLayoutImplFactoryTest,
                        testing::ValuesIn(factoryInputs));
...
TEST_P(GridLayoutConstructorTest, ConstructorTest)
{
    ...
    ASSERT_ANY_THROW(GridLayout(inputs.dxdydz, inputs.nbrCells, inputs.nbDim, inputs.layoutName,
